...
import os
from AlexNet.utils import train_one_epoch, evaluate, plot_class_preds
import argparse
...
def main(args):
    ...
    net =AlexNet(num_classes=args.num_classes).to(device)
    # 将模型写入tensorboard
    init_img = torch.zeros((1, 3, 224, 224), device=device,requires_grad=True)
    ...
    for epoch in range(args.epochs):
        ...
        if acc > best_acc:
            best_acc = acc
            torch.save(net.state_dict(), "./weights/modelAlexNet.pth")
